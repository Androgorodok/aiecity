...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, min_missing_share: float = 0.7) -> Dict[str, Any]:
    """
    ...
    for colonka in summary.columns:
        if colonka.unique == 1:
            flags["has_constant_columns"] = True
        if ('user_id' in colonka.name.lower()):
            if colonka.unique < summary.n_rows:
    ...
    if flags["has_constant_columns"] == True:
        score -= 0.05
    if flags["has_suspicious_id_duplicates"] == True:
        score -= 0.2
