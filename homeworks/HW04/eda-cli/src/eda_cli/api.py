...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict:
    """
    Эндпоинт возвращает полный набор флагов качества из CSV-файла.
    Включает все эвристики, реализованные в compute_quality_flags.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    # Фильтруем только флаги (булевы значения)
    flags_dict = {
        key: bool(value)
    ...
    # Добавляем числовые метрики (опционально)
    numeric_metrics = {
        key: value
        for key, value in quality_flags.items()
        if isinstance(value, (int, float)) and key != "quality_score"
    }
    
    print(
        f"[quality-flags-from-csv] filename={file.filename!r} "
        f"n_rows={summary.n_rows} n_cols={summary.n_cols} "
        f"flags_count={len(flags_dict)}"
    )
